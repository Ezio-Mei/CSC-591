...
def get_dataloader(data_path, labeled_size=200, mu=4, load_mode='semi'):
    ...
    dev_df = pd.read_csv(os.path.join(data_path,'dev_{}.csv'.format(labeled_size)))
    ...
    if load_mode == 'semi':
        ...
        train_loader_u = DataLoader(dataset=train_dataset_u, batch_size=8, shuffle=True, collate_fn=MyCollator(tokenizer))
    ...
    train_loader_l = DataLoader(dataset=train_dataset_l, batch_size=8, shuffle=True, collate_fn=MyCollator(tokenizer))
    
    dev_loader = DataLoader(dataset=dev_dataset, batch_size=16, shuffle=False, collate_fn=MyCollator(tokenizer))
    test_loader = DataLoader(dataset=test_dataset, batch_size=16, shuffle=False, collate_fn=MyCollator(tokenizer))
    
    num_class = max(train_l_df['label'].to_list())
    print(train_loader_l, dev_loader, test_loader, num_class)
